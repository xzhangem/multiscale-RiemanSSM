...
def H2UnparamKMean(samples,template,a0,a1,b1,c1,d1,a2,paramlist, N=None, geodesics=False, geods_len=2):
    if N is None:
        N=len(samples)
    ...
    for i in range(0,N):
        ...
        geod,F0=H2StandardIterative([mu,F0],sample,a0,a1,b1,c1,d1,a2,paramlist,N=geods_len)
        k=geod.shape[0]
    ...
    if geodesics:
        ...
        for i in range(0,len(samples)):
            ...
            geod,F0=H2StandardIterative([mu,F0],[samples[i][0],samples[i][1]],a0,a1,b1,c1,d1,a2,paramlist,N=geods_len)
            geods[i]=geod
        return geods,mu,F0
    return mu,F0
...
def H2_UnparamPCA(V0,samples,F0,a0,a1,b1,c1,d1,a2,paramlist,components=1,tol=None, geods=None,st_devs=1,geods_len=2):
    N=len(samples)        
    T=np.zeros((len(samples), V0.shape[0],3))
    if geods is None:
        for i in range(0,len(samples)):
            print('Computing Tangent Vector in the direction of sample {}/{}'.format(i+1,N))
            geod,F0=H2StandardIterative([V0,F0],[samples[i][0],samples[i][1]],a0,a1,b1,c1,d1,a2,paramlist,N=geods_len)
            k=geod.shape[0]
            T[i,:]=(geod[1]-geod[0])
    else:
        for i in range(0,len(samples)):
    ...
    #PCf=[]
    if tol is not None:
        evector_matrix = np.zeros(evalue.shape[0], M.shape[1], 3)
        for i in range(0,evalue.shape[0]):
            evector=np.zeros((M.shape[1],3))
            for j in range(0,evalue.shape[0]):
                evector+=evector_d[i,j]*M[j]
            evector_matrix[i,:,:] = evector
            
            print(evector.shape)
            if evalue[i]>tol:
                PC1p,Fp=H2InitialValueProblem(V0,k*evalue[i]*evector,k,a0,a1,b1,c1,d1,a2,F0)
                PCp_V.append(PC1p)
                PCp_F.append(Fp)
                PC1n,Fn=H2InitialValueProblem(V0,-1*k*evalue[i]*evector,k,a0,a1,b1,c1,d1,a2,F0)
                PCn_V.append(PC1n)
                PCn_F.append(Fn)
                #PC1=np.concatenate((np.flip(PC1n,axis=0),PC1p[1:]),axis=0)
                #PCf1=np.concatenate((np.flip(Fn,axis=0),Fp[1:]),axis=0)
                #PCs+=[PC1]
                #PCf+=[PCf1]

        return evalue,evector_matrix,PCp_V, PCp_F, PCn_V, PCn_F 
    else:
        evector_matrix = np.zeros((components, M.shape[1], 3))
...
def H2PCA(V0,samples,F0,a0,a1,b1,c1,d1,a2,paramlist,components=1,tol=None, geods=None, st_devs=1):
    ...
    if tol is not None:
        for i in range(0,evalue.shape[0]):
            ...
            print(evector.shape)
            if evalue[i]>tol:
                PC1p,F=H2InitialValueProblem(V0,k*evalue[i]*evector,k,a0,a1,b1,c1,d1,a2,F0)
                PC1n,F=H2InitialValueProblem(V0,-1*k*evalue[i]*evector,k,a0,a1,b1,c1,d1,a2,F0)
                PC1=np.concatenate((np.flip(PC1n,axis=0),PC1p[1:]),axis=0)
                PCs+=[PC1]
